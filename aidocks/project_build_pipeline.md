...
- **compile:** mirror version into `version.nvgt`, run `nvgt -c -Q tm.nvgt` from `src/` (bundle lands in `src/tm`), copy the `ASSET_FOLDERS` list — `data`,`docks`,`lib`,`sounds` — from `tm/` into the bundle (each via `copytree(..., dirs_exist_ok=True)`), move it to `releases/windows/ToyMania_windows_portable_password_is_<pw>/tm`. **`lib` MUST be in that list** (`build/tools.py`): it holds the runtime DLLs (nvgt_curl plugin, BASS/bass_fx/bassmix, phonon, Tolk/SAAPI64/nvda/zdsr) plus `7zr.exe` and `lame.exe`, which the game calls cwd-relative as `lib/7zr.exe` (updater, `updater.nvgt`) and `lib/lame.exe` (recording, `menu.nvgt`). It was omitted before 2026-08-15, so compiled builds shipped without the plugin/tools and the updater + recording broke; fixed by adding `"lib"` to `ASSET_FOLDERS`. The copy loop now treats `lib` as OPTIONAL (skips it if the folder is absent) so the same tools.py works for the sibling tsatria03 games that ship no `lib/` (ClassicStand, CookieCraze); ToyMania's `tm/lib` is present and always copied.

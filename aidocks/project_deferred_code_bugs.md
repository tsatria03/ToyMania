...
## 2. `writedata()` called on every footstep — FIXED 2026-08-14
Each of the 8 movement blocks in `game.nvgt` (tap + held) called `update_achievements()` then `writedata()`. `writedata()` (`src/includes/main/functions/savefuncts.nvgt:89`) rebuilds a ~20-key dictionary plus every achievement tier and calls `sd.save()` (serialize + encrypt + disk write); so every single step did a full encrypted save-file write — even though `savedata_timer`/`save_interval` (5000ms) already persists periodically.

FIX APPLIED: removed the per-step `writedata();` line from all 8 movement blocks, KEEPING `update_achievements()` (verified it does NOT call `writedata()` internally — `achievments.nvgt:91`, so unlocks including the Steps achievement still fire in-memory on each step). Persistence now comes from the periodic 5s timer save (`game.nvgt`, the `savedata_timer.elapsed >= save_interval` block) plus the death/game-over save and the event-based saves on entity kills/collections. Worst-case crash loss is up to ~5s of step-count drift.

Status as of 2026-08-14: #1, #1b, and #2 all FIXED. No open internal bugs currently tracked here.

...
- [Game unlock plan](project_game_unlock_plan.md) — IMPLEMENTED (sections 1-4; docks pending). Toy Defender MAIN game locked until 5 completed Collector games (either sub-mode); Endless Arcade after 3 Time Trials; Endless Defense after 3 Timed Defenses (Timed Defense opens with the Defender main game). Per-sub-mode difficulty ladder (each tier after 1 completed game of the tier below). Trigger = any end state except Escape-quit. Persisted per-sub-mode counters + a one-time migrate_unlocks() that grants pre-update veterans full access.
